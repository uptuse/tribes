// @ai-contract
// PURPOSE: Main rendering monolith — buildings, interiors, players,
//          projectiles, flags, weapons, camera, quality tiers,
//          spectator mode, and the core render loop
// SERVES: Scale, Belonging, Aliveness, Adaptation (contains subsystems for all four)
// DEPENDS_ON: three (+ 7 addons), renderer_terrain.js (Terrain.*),
//   renderer_postprocess.js (PostProcess.*), renderer_particles.js (Particles.*),
...
// LIFECYCLE: start() → Terrain.init → initBuildings → initInteriors → initPlayers →
...
// R32.232: heightmap state + Terrain.sampleHeight moved to renderer_terrain.js
// Use Terrain.sampleHeight(), Terrain.getHeightmap(), Terrain.getMesh()
...
export async function start() {
    ...
    await Terrain.init({ renderer, scene, Module });
    // R32.169: pass terrainMesh to DayNight now that it exists
    DayNight.setRef('terrainMesh', Terrain.getMesh());
    ...
    await Interiors.init({ scene, registerModelCollision }); // R32.233: extracted to renderer_interiors.js
    ...
    // try { Terrain.carveUnderBuildings(Interiors.getGroup()); } catch (e) { console.warn('[R32.140] terrain carve failed:', e); }
    ...
    polish = Polish.installPolish({
        ...
        sunLight: sunLight, hemiLight: hemiLight, terrainMesh: Terrain.getMesh(),
        sampleTerrainH: Terrain.sampleHeight,
    ...
    try {
        if (Interiors.getGroup()) {
            Interiors.getGroup().traverse(child => {
...
// R32.232: _splatData moved to renderer_terrain.js — use Terrain.getSplatData()
...
async function initBuildings() {
    ...
    for (let b = 0; b < count; b++) {
        ...
        const terrainAtBuilding = Terrain.sampleHeight(px, pz);
...
function initCustomModels() {
    ...
    const wolfX = -256.5;
    const wolfZ = 10.35;
    const wolfY = Terrain.sampleHeight(wolfX, wolfZ);
...
function loop() {
    ...
    // R32.232: terrain shader uTime tick moved to renderer_terrain.js
    Terrain.tick(t);
...
function initGrassRing() {
    ...
    if (!Terrain.getSplatData() || !Terrain.getHeightmap().data) {
        console.warn('[R32.32.1] initGrassRing aborted: terrain data missing');
        return;
    }
    const splat = Terrain.getSplatData().splatAttr;
    const splatN = Terrain.getSplatData().size;
    if (!splat || !splatN) {
        console.warn('[R32.32.1] initGrassRing aborted: splatAttr/size missing on getSplatData()', Terrain.getSplatData());
        return;
    }
    ...
    const span = (Terrain.getHeightmap().size - 1) * Terrain.getHeightmap().scale;
    const half = span * 0.5;
    ...
    for (let i = 0; i < N; i++) {
        ...
        const wy = Terrain.sampleHeight(wx, wz);
...
function initDustLayer() {
    ...
    return;
    if (Terrain.getHeightmap().size < 2) {
    ...
    for (let i = 0; i < N; i++) {
        ...
        const groundY = Terrain.sampleHeight(wx, wz);
        positions[i * 3 + 0] = wx;
...
function updateDustLayer(t) {
    ...
    for (let i = 0; i < N; i++) {
        ...
        positions[i * 3 + 0] = nx;
        positions[i * 3 + 2] = nz;
        positions[i * 3 + 1] = Terrain.sampleHeight(nx, nz) + st.hoverY[i];
    }
    ...
    for (let k = 0; k < REH; k++) {
        ...
        positions[i * 3 + 1] = Terrain.sampleHeight(st.homeX[i], st.homeZ[i]) + st.hoverY[i];
    }
